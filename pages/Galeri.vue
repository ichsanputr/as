            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-[2rem]">
                <div v-for="(video, key) in videos" :key="key" class="w-full rounded-lg">
                    <a :href="video.url" target="_blank" rel="noreferrer">
                        <iframe class="w-full rounded-t-lg shadow-sm" width="100%" height="245" loading="lazy" :src="video.url" title="YouTube video player"
                    ...
                    <div class="rounded-b-lg py-3 px-2 font-medium text-base md:text-lg backdrop-blur-sm bg-white/30 shadow-sm border border-slate-100">
                        <span v-if="video.description.length > 40 && $vuetify.display.mobile">{{ video.description.slice(0, 40) }}...</span>
